...
const STEPS: StepDef[] = [
  ...
  { id: 'tickets', labelKey: 'builder.steps.tickets', render: <TicketsStep />, when: (d) => d.modules.tickets },
...
export function EventNewPage() {
  ...
  // Reset the draft when leaving the wizard.
  useEffect(() => reset, [reset]);

  // Active steps recompute whenever modules change → dynamic flow.
  const activeSteps = useMemo(() => STEPS.filter((s) => !s.when || s.when(draft)), [draft]);

  // Keep the cursor valid if the active set shrinks under it.
  ...
  const create = () => {
    toast.success(t('builder.createdToast', { name: draft.name || 'New event' }));
    reset();
    navigate(paths.event('evt_summit'));
  };

  return (
    <div className="mx-auto max-w-[1340px] px-[34px] pb-16 pt-7">
      <button onClick={() => navigate(paths.events)} className="mb-4 flex items-center gap-1.5 text-[12.5px] text-text-secondary hover:text-text">
        <ChevronLeft size={14} /> {t('nav.events')}
      </button>
      ...
      <div className="mt-7 grid grid-cols-1 gap-7 lg:grid-cols-[1fr_360px]">
        ...
        <div>
          ...
          <div className="mt-5 flex justify-between">
            ...
            {isLast ? (
              <Button variant="primary" onClick={create} disabled={!draft.name.trim()}>
                {t('builder.review.createEvent')}
              </Button>
            ) : (
